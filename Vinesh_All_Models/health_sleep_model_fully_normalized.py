...
predictions = []
unique = []
min_labels = min(y["sleep_mgt"])
max_labels = max(y["sleep_mgt"])
...
for i in range(0, len(unique)):
    ...
    for x in range(0, len(y_train["sleep_mgt"])):
        y_train["sleep_mgt"].values[x] = (y_train["sleep_mgt"].values[x] - min_labels) / (max_labels - min_labels)

    # dont really need this, but just for consistency
    for k in range(0, len(y_test["sleep_mgt"])):
        y_test["sleep_mgt"].values[k] = (y_test["sleep_mgt"].values[k] - min_labels) / (max_labels - min_labels)

    for a in range(0, len(final_y_val["sleep_mgt"])):
        final_y_val["sleep_mgt"].values[a] = (final_y_val["sleep_mgt"].values[a] - min_labels) / (max_labels - min_labels)
