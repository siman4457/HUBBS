...
data = pd.read_csv("Non_uniform_health_tobacco_mgt_feature_final.csv", header=0)
...
normalized_X["ID"] = ID
# convert to binary
for i in range(0, len(y["tobacco_mgt"])):
    if int(y["tobacco_mgt"][i]) > 0:
        y["tobacco_mgt"].at[i] = 1
    else:
        y["tobacco_mgt"].at[i] = 0
...
def unweighted_avg(predictions, labels, threshold):
    ...
    one_total = 0
    one_correct = 0
    for x in range(0, len(duplicate)):
        if (labels[x] == 0):
            zero_total += 1
...
# use confusion matrix to calculate unweighted accuracy
want = y.drop(["ID"], axis=1)
labels = []
for i in range(0, len(want)):
    labels.append(want["tobacco_mgt"].at[i])
