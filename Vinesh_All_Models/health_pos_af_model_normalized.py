...
data = pd.read_csv("Non_uniform_health_pos_af_mgt_feature_final.csv")
...
predictions = []
unique = []
min_labels = min(y["pos_af_mgt"])
max_labels = max(y["pos_af_mgt"])
...
for i in range(0, len(unique)):
    ...
    for x in range(0, len(y_train["pos_af_mgt"])):
        y_train["pos_af_mgt"].values[x] = (y_train["pos_af_mgt"].values[x] - min_labels) / (max_labels - min_labels)

    # dont really need this, but just for consistency
    for k in range(0, len(y_test["pos_af_mgt"])):
        y_test["pos_af_mgt"].values[k] = (y_test["pos_af_mgt"].values[k] - min_labels) / (max_labels - min_labels)

    for a in range(0, len(final_y_val["pos_af_mgt"])):
        final_y_val["pos_af_mgt"].values[a] = (final_y_val["pos_af_mgt"].values[a] - min_labels) / (max_labels - min_labels)
