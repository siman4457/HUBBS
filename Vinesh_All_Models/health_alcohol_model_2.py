...
for i in range(0, len(y["alcohol_mgt"])):
    if int(y["alcohol_mgt"][i]) > 0:
        y["alcohol_mgt"].at[i] = 1
    else:
        y["alcohol_mgt"].at[i] = 0
...
def unweighted_avg(predictions, labels, threshold):
    ...
    one_total = 0
    one_correct = 0
    for x in range(0, len(labels)):  # total zeros in labels not preds
        if labels[x] == 0:
            zero_total += 1
...
for i in range(0, len(unique)):
    ...
    for a in range(0, len(prediction)):
        preds.append(prediction[a][0])

want = y.drop(["ID"], axis=1)
labels = []
for i in range(0, len(want)):
    labels.append(want["alcohol_mgt"].at[i])
